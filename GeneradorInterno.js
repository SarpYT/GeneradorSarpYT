    ...
    var miniaturaInput = document.getElementById("miniatura");
      ...
      generarButton.addEventListener("click", function() {
        ...
        var miniatura = miniaturaInput.value.trim();
...
.Trsa:hover {background:rgba(0, 255, 0,.5);}
	...
	.plyr__poster {
  border-radius: 10px; /* Puedes ajustar el valor según el redondeo deseado */
  
}

.plyr {
  border-radius: 20px; /* Puedes ajustar el valor según el redondeo deseado */
  width: auto;
  height: auto;
}
...
<input type="checkbox" id="ventana-checkbox"><div class="ventana-emergente"> <center><h3 class="Tituloxd">Escoge Tu Servidor</h3> </center>


<!-- AnimeDay Player -->
  
  <video id="player" controls poster="${miniatura}">
    <source src="${enlace1}" type="video/mp4">
  </video><br>
...
function abrirD() {
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      rewindButton.className = 'plyr__control plyr__control--rewind';
      rewindButton.title = 'Rebobinar';

      var fastForwardButton = document.createElement('button');
      fastForwardButton.innerHTML = '<i class="fas fa-forward"></i>';
      fastForwardButton.className = 'plyr__control plyr__control--fast-forward';
      fastForwardButton.title = 'Avance rápido';
